...
- **Nav bar**: fixed `<nav>` with a `.nav-brand` (SVG logo mark + "Enchanted" wordmark) and a `.nav-links` list linking to `index.html`, `library.html`, `calculator.html`, `glp1-calculator.html`, `sop-library.html`, `lab-values.html`. The current page's link gets `class="active"`.
  - **Known drift**: `calculator.html`'s nav only links to Home/Library/Calculator — it's missing the GLP-1/SOPs/Labs links the other pages have. `index.html`'s tile grid also only links to `library.html` and `calculator.html`, not the newer `glp1-calculator.html`, `sop-library.html`, or `lab-values.html` pages. Keep this in mind: don't assume the nav or homepage tiles are exhaustive, and consider fixing the drift if you're touching navigation.
